...
class IsoRender:
    """
    Rendre isometrique basique (2.5D).
    - Affichage en vue isometrique
    - Animations simples
    - Barres de vie
    """
    ...
    def __init__(self, tile_size=48):
        ...
        pygame.display.set_caption("BattleSimulation 2.5D")

        self.clock = pygame.time.Clock()

        #Couleurs
        self.COLOR_GROUND = (200,180,140)
        self.COLOR_UNIT_P0 = (120,180,255)
        self.COLOR_UNIT_P1 = (255,140,120)
        self.COLOR_HP = (0,255,0)
        self.COLOR_HP_BG = (100,0,0)

    #Conversion coordonnees isometriques
    def iso(self, x, y):
        ts = self.tile_size
        iso_x = (x-y)*ts+450
        iso_y = (x+y)*(ts//2)+80
        ...
        for y in range(game.height):
            for x in range(game.width):
                ...
                pygame.draw.polygon(
                    ...
                    [
                        (px,py),
                        (px+self.tile_size, py+self.tile_size//2),
                        (px, py+self.tile_size),
                        (px-self.tile_size, py+self.tile_size//2),
                    ]
                )
    ...
    def draw_unit(self, unit):
        ...
        #Corps de l'unit
        color = self.COLOR_UNIT_P0 if unit.owner == 0 else self.COLOR_UNIT_P1

        body_w = self.tile_size//2
        body_h = self.tile_size//2

        pygame.draw.rect(
            self.screen,
            color,
            (px-body_w//2, py-body_h, body_w, body_h)
        )
        ...
        pygame.draw.rect(
            self.screen, self.COLOR_HP_BG,
            (px-hp_w//2, py-body_h-10, hp_w, hp_h)
        )
        pygame.draw.rect(
            self.screen, self.COLOR_HP,
            (px-hp_w//2, py-body_h-10, hp_w*hp_ratio, hp_h)
        )
    ...
    def render(self, game):
        ...
        #Dessiner units
        for unit in sorted(game.units, key=lambda t:(t.pos[0]+t.pos[1])):
            if unit.is_alive:
                self.draw_unit(unit)
